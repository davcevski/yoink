...
An **encrypted** macOS clipboard history with an **8-bit TUI**.
...
## Global hotkey & launchers

yoink is a terminal app, so it can't grab a global hotkey on its own (that needs
a native menu-bar app or an Accessibility event tap). **Spotlight** also can't be
extended with live third-party results. But you can get a global hotkey and a
"yoink: my keywords" experience with tools you may already have, using the
non-interactive `yoink search` / `yoink copy` commands:

```sh
yoink search git           # -> "<id>\t<preview>" lines for matching clips
yoink search --json git    # -> JSON array (for launchers)
yoink copy 42              # -> put clip #42 back on the clipboard
```

**Alfred** (best "yoink: my keywords" experience — a Script Filter):

```bash
# Script Filter, keyword: yoink, "with input as {query}", language /bin/bash
yoink search --json "$1" | /usr/bin/python3 -c '
import sys, json
clips = json.load(sys.stdin)
items = [{"uid": str(c["id"]), "title": c["preview"],
          "subtitle": f"{c[\"bytes\"]} B — ⏎ to copy", "arg": str(c["id"])}
         for c in clips] or [{"title": "No matches", "valid": False}]
print(json.dumps({"items": items}))'
```

Connect its output to a **Run Script** action: `yoink copy "{query}"`. Now
`yoink some keywords` in Alfred filters live and ⏎ copies the pick back.
...
# @raycast.title Search clipboard (yoink)
# @raycast.mode fullOutput
# @raycast.argument1 { "type": "text", "placeholder": "keywords" }
yoink search "$1"
```

**Global hotkey to the full TUI** (no extra commands needed):

- **iTerm2 hotkey window:** Settings → Keys → Hotkey → create a dedicated hotkey
  window whose profile runs `yoink`. Hit the hotkey → a dropdown terminal opens
  yoink → search → ⏎ → it's back on your clipboard.
- **skhd:** bind a key to open a terminal running yoink, e.g.
  `cmd + shift - v : osascript -e 'tell app "iTerm" to create window with default profile command "yoink"'`
- **macOS Shortcuts:** a "Run Shell Script" shortcut works well with the
  non-interactive `yoink search`/`yoink copy` commands (the full TUI needs a TTY).
